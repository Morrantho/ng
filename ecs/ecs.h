...
void ng_ecs_system(register ng_u64_t mask,register ng_ecs_system_fn_p fn)
{
	...
	g_ng_ecs.systems[system_id].mask=mask;
	g_ng_ecs.systems[system_id].run=fn;
	system_id++;
}

void ng_ecs_component_toggle(register ng_ecs_entity_t e,register ng_ecs_component_t c)
{
	g_ng_ecs.entities[e]^=1<<c;
	register ng_u64_t em=g_ng_ecs.entities[e];
	register ng_u8_t s;
	for(s=0;s<NG_ECS_SYSTEM_MAX;s++)
	{
		register ng_u64_t sm=g_ng_ecs.systems[s].mask;
		register ng_u64_t r=em&sm;
		g_ng_ecs.systems[s].entities[e]=(r==em||r==sm)&&r!=1;
	}
}
...
void ng_ecs_think(register ng_u8_t type)
{
	...
	for(s=0;s<NG_ECS_SYSTEM_MAX;s++)
	{
		if(!g_ng_ecs.systems[s].run) continue;
		for(e=0;e<NG_ECS_ENTITY_MAX;e++)
		{
			if(g_ng_ecs.systems[s].entities[e])
			{
				g_ng_ecs.systems[s].run(e);
			}
		}
	}
}
